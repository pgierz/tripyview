...
def create_3dsphere_ocean_mesh(mesh, data, potatoefac=0.5,variable='elevation', do_texture=False):
    ...
    if all(
        x not in vname
        for x in ['depth', 'topo', 'topography', 'zcoord', 'bathymetry']
    ):    
...
def create_3dsphere_lonlat_grid(dlon=30,dlat=15,potatoefac=1.0,do_topo=False):
    ...
    for nlonline in range(len(grid_lon)):
    ...
    for nlatline in range(len(grid_lat)):
...
def create_3dsphere_0lon0lat_grid(dlon=30,dlat=15,potatoefac=1.0,do_topo=False):
    ...
    grid_lon = [0]
    dum_lat  = np.arange(-85,85+1,1)
    for item in grid_lon:
        xs, ys, zs = (
            grid_cart3d(
                dum_lat * 0 + item,
                dum_lat,
                R_earth + (6000 * 100 * potatoefac),
                is_deg=True,
            )
            if do_topo
            else grid_cart3d(
                dum_lat * 0 + item, dum_lat, R_earth * 1.005, is_deg=True
            )
        )
        points     = np.vstack((xs,ys,zs)).transpose()
        aux_points = np.column_stack((points[:-1,:],points[1:,:]))
    ...
    for item_ in grid_lat:
        xs, ys, zs = (
            ...
            if do_topo
                ...
                dum_lon, dum_lon * 0 + item_, R_earth * 1.005, is_deg=True
            )
        )
        points     = np.row_stack((xs,ys,zs)).transpose()
        aux_points = np.column_stack((points[:-1,:],points[1:,:]))
...
def create_3dflat_ocean_wall(xs, ys, zs, e_i, which_wall='N', nsigma=20):
    print(f' --> compute 3d flat ocean {which_wall} wall')
    #___________________________________________________________________________
    ...
    for ii in range(nsigma-1):
        ...
        wall_sigma_e_i = aux_e_i if ii==0 else np.vstack((wall_sigma_e_i,aux_e_i))
    #___________________________________________________________________________
...
def create_3dflat_ocean_botwall(xs, ys , zs, e_i, box, scalfac, botdepth=7000 ):
    ...
    return {'xs': box_xs, 'ys': box_ys, 'zs': box_zs, 'e_i': box_e_i}
...
def create_3dflat_land_mesh(mesh, resol=1, box=None, do_topo=False, topo_path=[], 
    ...
    if box is not None:
        ...
        land_botwall = {'xs': box_xs, 'ys': box_ys, 'zs': box_zs, 'e_i': box_e_i}        
            ##_______________________________________________________________________
