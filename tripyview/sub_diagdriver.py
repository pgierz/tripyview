...
def drive_hslice(yaml_settings, analysis_name):
    ...
    for vname in driver_settings:
        ...
        for depth in driver_settings[vname]["depths"]:
            ...
            current_params2 |= driver_settings[vname]
            ...
            if 'months' not in driver_settings[vname].keys(): 
                ...
                webpage[f"image_{image_count}"] = {
                    "name": f"{auxvname.capitalize()} @dep:{depth}m"
                }
                ...
                for month in driver_settings[vname]["months"]:
                    ...
                    #___________________________________________________________
                    webpage[f"image_{image_count}"] = {
                        "name": f"{auxvname.capitalize()} @dep:{depth}m, @mon:{month}"
                    }
                    webpage[f"image_{image_count}"]["path"]       = os.path.join('./figures/', save_fname)
                    webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
                    webpage[f"image_{image_count}"]["short_name"] = short_name
...
def drive_hslice_clim(yaml_settings, analysis_name):
    ...
    for vname in driver_settings:
        ...
        for depth in driver_settings[vname]["depths"]:
            ...
            #___________________________________________________________________
            webpage[f"image_{image_count}"] = {
                "name": f"{vname.capitalize()} at {depth} m"
            }
            webpage[f"image_{image_count}"]["path"]       = os.path.join('./figures/', save_fname)
            webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
            webpage[f"image_{image_count}"]["short_name"] = short_name
...
def drive_hslice_isotherm_z(yaml_settings, analysis_name):
    ...
    for which_isotherm in driver_settings['which_isotherms']:
        ...
        current_params2 |= driver_settings
        ...
        if 'months' not in driver_settings.keys(): 
            ...
            #_______________________________________________________________
            webpage[f"image_{image_count}"] = {
                "name": f"depth of {which_isotherm}°C isotherm",
                "path": os.path.join('./figures/', save_fname),
            }
            webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
            webpage[f"image_{image_count}"]["short_name"] = short_name
            image_count += 1
            ...
            for month in driver_settings["months"]:
                ...
                #___________________________________________________________
                webpage[f"image_{image_count}"] = {
                    "name": f"depth of {which_isotherm}°C isotherm, @mon:{month}",
                    "path": os.path.join('./figures/', save_fname),
                }
                webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
                webpage[f"image_{image_count}"]["short_name"] = short_name
...
def drive_hovm(yaml_settings, analysis_name):
    ...
    for vname in driver_settings:
        ...
        for box_region in driver_settings[vname]["box_regions"]:
            ...
            current_params2 = current_params.copy()
            current_params2["vname"] = vname
            current_params2["box_region"] = [box_region]
            current_params2 |= driver_settings[vname]
            ...
            #___________________________________________________________________
            webpage[f"image_{image_count}"] = {
                "name": f"{auxvname.capitalize()} at {str_boxregion} m"
            }
            webpage[f"image_{image_count}"]["path"] = os.path.join('./figures/', save_fname)
            webpage[f"image_{image_count}"]["path_nb"] = os.path.join('./notebooks/', save_fname_nb)
            webpage[f"image_{image_count}"][
...
def drive_hovm_clim(yaml_settings, analysis_name):
    ...
    for vname in driver_settings:
        ...
        for box_region in driver_settings[vname]["box_regions"]:
            ...
            #___________________________________________________________________
            webpage[f"image_{image_count}"] = {
                "name": f"{vname.capitalize()} at {str_boxregion} m"
            }
            webpage[f"image_{image_count}"]["path"] = os.path.join('./figures/', save_fname)
            webpage[f"image_{image_count}"]["path_nb"] = os.path.join('./notebooks/', save_fname_nb)
            webpage[f"image_{image_count}"][
...
def drive_zmoc(yaml_settings, analysis_name):
    ...
    webpage = {}
    # loop over variable name
    for image_count, vname in enumerate(driver_settings):
        print(f'         --> compute: {vname}')
...
def drive_zmoc_t(yaml_settings, analysis_name):
    ...
    webpage = {}
    # loop over variable name
    for image_count, which_lat in enumerate(driver_settings['which_lats']):
        ...
        #_______________________________________________________________________
        webpage[f"image_{image_count}"] = {
            "name": " max AMOC @ 30°N<lat<45°N"
            if which_lat == 'max'
            else f" AMOC @ {which_lat}°N",
            "path": os.path.join('./figures/', save_fname),
        }
        webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
...
def drive_dmoc(yaml_settings, analysis_name):
    ...
    for image_count, vname in enumerate(driver_settings):
        ...
        #_______________________________________________________________________
        webpage[f"image_{image_count}"] = {
            "name": f"Density-{vname.upper()}{str_mode}"
        }
        webpage[f"image_{image_count}"]["path"]       = os.path.join('./figures/', save_fname)
        webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
        webpage[f"image_{image_count}"]["short_name"] = f"{yaml_settings['workflow_name']}_{analysis_name}_{vname}"
...
def drive_dmoc_t(yaml_settings, analysis_name):
    ...
    for image_count, which_lat in enumerate(driver_settings['which_lats']):
        ...
        #_______________________________________________________________________
        webpage[f"image_{image_count}"] = {
            "name": " max density-AMOC @ 45°N<lat<60°N"
            if which_lat == 'max'
            else f" density AMOC @ {which_lat}°N",
            "path": os.path.join('./figures/', save_fname),
        }
        webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
        webpage[f"image_{image_count}"]["short_name"] = f"{yaml_settings['workflow_name']}_{analysis_name}_{which_lat}"
...
def drive_dmoc_wdiap(yaml_settings, analysis_name):
    ...
    for image_count, which_isopyc in enumerate(driver_settings['which_isopycs']):
        ...
        #_______________________________________________________________________
        webpage[f"image_{image_count}"] = {
            "name": f"W_diap at sigma2={which_isopyc} kg/m^3",
            "path": os.path.join('./figures/', save_fname),
        }
        webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
        webpage[f"image_{image_count}"]["short_name"] = short_name
...
def drive_dmoc_srfcbflx(yaml_settings, analysis_name):
    ...
    for image_count, which_isopyc in enumerate(driver_settings['which_isopycs']):
        ...
        #_______________________________________________________________________
        webpage[f"image_{image_count}"] = {
            "name": f"srf. buoyancy transf. at sigma2={which_isopyc} kg/m^3",
            "path": os.path.join('./figures/', save_fname),
        }
        webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
        webpage[f"image_{image_count}"]["short_name"] = short_name
...
def drive_hbarstreamf(yaml_settings, analysis_name):
    ...
    #
    driver_settings = (
        yaml_settings[analysis_name].copy()
        if yaml_settings[analysis_name]
        else {}
    )
    current_params = {
        key: value
    ...
    image_count = 0
    print('         --> compute: hbarstreamf')
    current_params2 = current_params.copy() if current_params else {}
    current_params2 |= driver_settings
    ...
    webpage = {
        f"image_{image_count}": {
            "name": 'horiz. barotrop. streamfunction',
            "path": os.path.join('./figures/', save_fname),
        }
    }
    webpage[f"image_{image_count}"]["path_nb"]    = os.path.join('./notebooks/', save_fname_nb)
...
def drive_transect(yaml_settings, analysis_name):
    ...
    for vname in driver_settings2: 
        ...
        for transect in which_transects:
            ...
            current_params2 = current_params.copy()
            current_params2["vname"] = vname
            current_params2["input_transect"] = [transect]
            current_params2 |= driver_settings2[vname]
