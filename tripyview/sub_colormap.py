...
def categorical_cmap(nc, nsc, cmap="tab10", cmap2='nipy_spectral', continuous=False):
    ...
    if continuous:
        ccolors = (
            ListedColormap(
                plt.cm.get_cmap(cmap2, nc)(np.linspace(0, 1, nc))
            ).colors
            if nc > plt.get_cmap(cmap).N
            else plt.get_cmap(cmap)(np.linspace(0, 1, nc))
        )
    elif nc > plt.get_cmap(cmap).N:
